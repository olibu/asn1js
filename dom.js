...
;(typeof define != 'undefined'
  ...
  ASN1.prototype.toDOM = function (spaces) {
    spaces = spaces || ''
    var isOID =
      typeof oids === 'object' && ((this.tag.isUniversal() && this.tag.tagNumber == 0x06) || this.tag.tagNumber == 0x0d)
    ...
    if (this.sub !== null) {
      spaces += '\xA0 '
      for (var i = 0, max = this.sub.length; i < max; ++i) sub.appendChild(this.sub[i].toDOM(spaces))
    }
  ...
  ASN1.prototype.toHexDOM = function (root) {
    ...
    if (root == node) {
      ...
      if (lineStart != 0) {
        ...
        var skipStr = ''
        for (var j = lineStart; j > 0; --j) skipStr += '   '
    ...
    if (this.sub === null) {
      ...
      var end = this.posEnd()
      if (end - start < 10 * 16) node.appendChild(DOM.text(this.stream.hexDump(start, end)))
      else {
